...
```{r, out.width='100%'}
teachingApps::includeApp('probability_plotting', height = '600px')
```
...
```{r out.width='100%'}
teachingApps::includeApp('figure6_1', height = '600px')
```
...
```{r}
 teachingApps:::figure6_2(pub = T, rmd = T)
```
...
```{r}
 teachingApps:::figure6_3(pub = T, rmd = T)
```
...
```{r}
 teachingApps:::figure6_4(pub = T, rmd = T)
```
...
```{r}
 teachingApps:::figure6_5(pub = T, rmd = T)
```
...
```{r}
 teachingApps:::at7987_data(pub = T, rmd = T)
```
...
```{r}
 teachingApps:::shockabsorber_data6(pub = T, rmd = T)
```
</div>
...
```{r}
 teachingApps:::heatexchanger_data6(pub = T, rmd = T)
```
</div>
...
```{r}
 teachingApps:::turbine_data6(pub = T, rmd = T)
```
</div>
...
```{r, fig.align='center'}
 teachingApps:::bent_probplot(pub = T, rmd = T)
```
...
```{r,  results='asis'}
 teachingApps:::bleed_data6(pub = T, rmd = T)
```
</div>
...
```{r, eval=FALSE}
library(plotly)
p <- plot_ly(economics, x = date, y = unemploy / pop, width = '100%')
p
```


'<table>',
'<tr><td>','Pr(accept)','</td><td>',' = ','</td><td>',round(accept, digits = 5),'</td></tr>',
'<tr><td>','Failures Allowed','</td><td>',' = ','</td><td>',input$fails,'</td></tr>',
'<tr><td>','Total Test Time','</td><td>',' = ','</td><td>',input$ttt,'</td></tr>',
'<tr><td>','True MTBF','</td><td>',' = ','</td><td>',input$mtbf,'</td></tr>',
'</table>'
