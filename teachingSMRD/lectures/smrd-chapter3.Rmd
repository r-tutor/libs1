...
```{r}
shiny::includeCSS("../scripts/css/slidy-flatly.css")
...
```{r, out.width='100%'}
knitr::include_app(paste0(root_url,'example3_1'), height = '800px')
```
...
- Convention for this text - there is insufficient information to assign a value of $\widehat{F}(t)\in (t_{i-1},t_{i})$ for censored observations occuring between inspections
...
- Confidence intervals (CI) are useful tools for quantifying the uncertainty associated with a point estimate due to sampling errors
...
```{r, out.width='100%'}
knitr::include_app(paste0(root_url,'confidence_intervals'), height = '800px')
```
</div>
...
<smaller>
$$(\underline{F}(t_i),\overline{F}(t_i))=\left(1+\frac{(n-n\widehat{F}+1)\mathcal{F}_{(1-\alpha/2,2n-2n\widehat{F}+2,2n\widehat{F})}}{n\widehat{F}}\right)^{-1},
\left(1+\frac{n-n\widehat{F}}{(n\widehat{F}+1)\mathcal{F}_{(1-\alpha/2,2n\widehat{F}+2,2n-2n\widehat{F})}}\right)^{-1}$$
</smaller>
...
```{r, out.width='100%'}
knitr::include_app(paste0(root_url,'lfp1370_data'), height = '800px')
```
...
```{r echo=FALSE,fig.height=4,fig.width=7.5,fig.align='center'}
...
$$
\begin{aligned}
\widehat{S}(t_{i})&=\prod_{j=1}^{i}\left[1-\widehat{p_{j}}\right], i=1,...,m\\\\
\widehat{F}(t_{i})&=1-\prod_{j=1}^{i}\left[1-\widehat{p_{j}}\right], i=1,...,m
\end{aligned}
$$
...
par(family = "serif", font = 2, pin = c(6.5,5), lwd = 2)
plot(NA,
     axes = FALSE,
     xlab = "",
     ylab = "",
...
text(x = rep(-25, 3),
     y = seq(56, 256, 100),
     labels = c("Plant 3","Plant 2","Plant 1"),
     cex = rep(0.9,3))
...
<small>
$$
`r SMRD::xarray(Table.3.2, include.rownames = FALSE, digits = c(0,0,0,0,0,0,0,0,4,4), sanitize.text.function = function(x) {x})`
$$
</small>
...
$\mathbf{\theta}$, say $g(\mathbf{\widehat{\theta}})$
...
# 3.6.1 - Approximate Variance of $\widehat{F}(t_{i})$
...
$$
\frac{\partial S(t_{i})}{\partial q_{i}} \;=\;
\frac{\partial \left( q_{1}q_{2}...q_{i-1}q_{i}\right)}{\partial q_{i}}
\;=\; q_{1}q_{2}...q_{i-1}
\;=\; \frac{S(t_{i})}{q_{i}}
$$
...
$$
\begin{aligned}
\widehat{Var}(1)&=[\widehat{S}(t_{i})]^{2}\sum_{j=1}^{i}\left[\frac{\widehat{p}_{j}}{n_{j}(1-p_{j})}\right]\\
...
- Applying the logit transformation to $\widehat{F}(t_{i})$ gives
...
- A table of values for $\widehat{F}(t_{i})$ can be produced from the SMRD package using
...
knitr::include_app(paste0(root_url, 'table3_4'), height = '800px')
```
...
```{r, out.width='100%', eval=FALSE}
knitr::include_app(paste0(root_url, 'table3_5'), height = '800px')
```
